...
import { Clock, Plus, Trash2, LogOut, Play, Pause, CheckCircle2, Edit2, Tag, X, Save, FolderPlus, Briefcase, Calendar, BarChart2, Bell, BellOff, Settings } from 'lucide-react';
...
export default function App() {
  ...
  const lastNotified2hBlock = React.useRef<number>(0);
  ...
  useEffect(() => {
    ...
    if (runningTask) {
      const startedAt = runningTask.timerStartedAt?.toDate?.() || new Date();
      const continuousRunningSeconds = Math.floor((now.getTime() - startedAt.getTime()) / 1000);
      ...
      // --- NEW: Are you working? (Every 2 hours) ---
      const current2hBlock = Math.floor(continuousRunningSeconds / 7200);
      if (current2hBlock > 0 && current2hBlock > lastNotified2hBlock.current) {
        playNotificationSound();
        toast.warning('Таймер все еще идет. Вы работаете?', {
          description: `Задача "${runningTask.taskName}" выполняется уже ${current2hBlock * 2} часов подряд.`,
          duration: 30000,
        });
        sendTelegramNotification(`⚠️ Таймер все еще идет. Вы работаете?\nЗадача "${runningTask.taskName}" выполняется уже ${current2hBlock * 2} часов подряд.`);
        lastNotified2hBlock.current = current2hBlock;
      }

      // --- NEW: Overnight protection (8 hours) ---
      if (continuousRunningSeconds >= 8 * 3600) {
        stopTimer(runningTask);
        playNotificationSound();
        toast.error('Таймер автоматически остановлен', {
          description: `Задача "${runningTask.taskName}" выполнялась более 8 часов подряд.`,
          duration: 20000,
        });
        sendTelegramNotification(`🛑 Таймер автоматически остановлен!\nЗадача "${runningTask.taskName}" выполнялась более 8 часов подряд.`);
        return; // Exit early since we stopped it
      }

      // --- NEW: Auto-stop by limit (Planned + 30 mins) ---
      if (plannedSeconds > 0 && totalSeconds >= plannedSeconds + 1800) {
        stopTimer(runningTask);
        playNotificationSound();
        toast.error('Таймер автоматически остановлен', {
          description: `Задача "${runningTask.taskName}" превысила запланированное время на 30 минут.`,
          duration: 20000,
        });
        sendTelegramNotification(`🛑 Таймер автоматически остановлен!\nЗадача "${runningTask.taskName}" превысила запланированное время на 30 минут.`);
        return;
      }
      ...
      if (plannedSeconds > 0 && remainingSeconds > 0 && remainingSeconds <= 300 && !notifiedNearExpiration.current.has(runningTask.id)) {
        ...
        playNotificationSound();
        sendTelegramNotification(`⏳ Осталось 5 минут: ${runningTask.taskName}\nЗапланированное время почти истекло.`);
      ...
      if (isTaskOvertime && !notifiedTasks.current.has(runningTask.id)) {
        ...
        playNotificationSound();
        sendTelegramNotification(`⚠️ Время истекло: ${runningTask.taskName}\nНачался учет переработки.`);
      ...
      if (project && project.weeklyBudget > 0 && !notifiedWeeklyBudgets.current.has(project.id)) {
        ...
        if (projectStat && projectStat.week >= project.weeklyBudget) {
          ...
          playNotificationSound();
          sendTelegramNotification(`📊 Бюджет исчерпан: ${project.name}\nНедельный бюджет проекта полностью использован.`);
        ...
        {showSettings && (
          <div className="bg-white shadow rounded-lg mb-8 p-6 animate-in fade-in slide-in-from-top-4">
            ...
            <div className="space-y-4 max-w-lg">
              <div>
                <h4 className="text-md font-medium text-gray-900 mb-2">Уведомления в Telegram</h4>
                <p className="text-sm text-gray-500 mb-4">
                  Укажите токен бота и ваш Chat ID, чтобы получать уведомления об авто-остановке таймера и напоминания.
                </p>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        localStorage.setItem('tgToken', e.target.value);
                      }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        localStorage.setItem('tgChatId', e.target.value);
                      }}
                  ...
                  <button
                    onClick={() => {
                      sendTelegramNotification('✅ Тестовое уведомление из Трекера Времени успешно отправлено!');
